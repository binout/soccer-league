...
@Component
public class GetLeagueMatches {

    private final SeasonRepository seasonRepository;
    private final MatchPlanning matchPlanning;

    @Autowired
    public GetLeagueMatches(SeasonRepository seasonRepository, MatchPlanning matchPlanning) {
        this.seasonRepository = seasonRepository;
        this.matchPlanning = matchPlanning;
    }

    @Transactional
    public List<Tuple2<LeagueMatch, List<Player>>> execute(String seasonName) {
        Season season = seasonRepository.byName(seasonName).orElseThrow(() -> new IllegalArgumentException("Invalid season"));
        return season.leagueMatches().map(m -> Tuple.of(m, matchPlanning.getSubstitutes(season, m))).collect(Collectors.toList());
    }
}
